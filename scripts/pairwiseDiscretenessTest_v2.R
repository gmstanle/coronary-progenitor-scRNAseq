...
#' Perform pairwise continuity analysis on grouping of cells
#' Possible: sim genes by correlation?
#' pairwiseDiscretenessTest_v2: no longer calculating pvalues for each gene (takes forever)
pairwiseDiscretenessTest_v2 <- function(object, subsample.genes=F,
  ...
  pair.res <- foreach(i = 1:nrow(ids.iterator), .combine="c") %dopar% {
    ...
    ident.res <- scoreCellsByIdentitySignature(object=pairObject, 
                                               id1 = id1, 
                                               id2 = id2, 
    ...
    # if there are sufficient diff expr genes, calculate the discrete statistics and 
    # fit the 5-parameter model of cell identity (2-Gaussian + intermediates)
    else{
    ...
    print(ident.signature)
    return(list(c(id1, id2), dstats, cellid.params, ident.signature))#, genes.1, genes.2)
...
scoreCellsByIdentitySignature <- function(object, id1, id2, num.sig.genes=30, method = 'pct_or_diff', 
                                          genes.use=NULL, dir=getwd()){
...
PlotConnectogram <- function(object, pair.res, colorvec=NULL, median.disc.cutoff = -6,
  ...
  #   
  plot(nodes$tSNE_1, nodes$tSNE_2, col=as.character(nodes$color), cex=sqrt(nodes$numcells)*.35, pch=19, asp=1) +
    with(edges, segments(x0,y0,x1,y1, lwd = width, col = alpha('black', edges$alph))) +
    points(nodes$tSNE_1, nodes$tSNE_2, col=as.character(nodes$color), cex=sqrt(nodes$numcells)*.35, pch=19)
    points(nodes$tSNE_1, nodes$tSNE_2, col="black", cex=sqrt(nodes$numcells)*.35, pch=1)
